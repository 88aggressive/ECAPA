...
class ECAPAModel(nn.Module):
	def __init__(self, lr, lr_decay, C , n_class, m, s, test_step, **kwargs):
		...
		#ECAPA_TDNN(C = C) = torch.nn.parallel.DistributedDataParallel(ECAPA_TDNN(C = C), device_ids=["0,2,3"])
	...
	def train_network(self, epoch, loader):
		...
		for num, (data, labels) in enumerate(loader, start = 1):
			...
			speaker_embedding = self.speaker_encoder.forward(data.to(device), aug = True)
	...
	def eval_network(self, eval_list, eval_path,eval_list1, eval_path1):
		self.eval()
		files = []
		embeddings = {}
		...
		for idx, file in tqdm.tqdm(enumerate(setfiles), total = len(setfiles)):
			audio, sr = torchaudio.load(os.path.join(eval_path, setfiles[idx]))
			# Full utterance
